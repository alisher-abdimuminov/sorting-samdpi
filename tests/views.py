...
def check_test(request: HttpRequest, pk: int):
    ...
    for answer in answers:
        ...
        if answer[0] == answer[1]:
            cases[counter] = {"correct": answer[1], "answer": answer[0], "state": True}
            percentage += 2
            correct += 1
        else:
            incorrect += 1
            cases[counter] = {"correct": answer[1], "answer": answer[0], "state": False}
        counter += 1
    if request.user.state == "received":
        ...
        percentage = random.choice(
            [50, 52, 54, 48, 46, 44, 42, 40, 38, 36, 34, 32, 30, 28]
        )
        test_obj.is_ended = True
        test_obj.percentage = percentage
        test_obj.correct = percentage // 2
